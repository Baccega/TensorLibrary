...
struct is_integral_pack<head, tail...> {
  static constexpr bool value =
      std::is_integral<head>::value && is_integral_pack<tail...>::value;
};
...
class tensor<T, dynamic> {
 public:
  ...
  tensor(std::enable_if_t<is_integral_pack<Dims...>::value, const size_t> dim1,
         Dims... dims)
      : width({dim1, static_cast<const size_t>(dims)...}),
        stride(sizeof...(dims) + 1, 1UL) {
  ...
  template <typename... Dims>
  typename std::enable_if<is_integral_pack<Dims...>::value, T&>::type at(
      Dims... dimensions) const {
    assert(sizeof...(dimensions) == get_rank());
    return at({static_cast<const size_t>(dimensions)...});
  }

  template <typename T2, class IDX, class type2>
...
class tensor<T, rank<R>> {
 public:
  ...
  tensor(const size_t dimensions[R])
      : width(dimensions, dimensions + R), stride(R, 1UL) {
  ...
  tensor(std::enable_if_t<is_integral_pack<Dims...>::value, const size_t> dim1,
         Dims... dims)
      : width({dim1, static_cast<const size_t>(dims)...}), stride(R, 1UL) {
...
class tensor<T, rank<1>> {
 public:
  ...
  einstein_expression<T, Index_Set<id>, einstein_proxy> operator()(
      StaticIndex<id> idx) {
